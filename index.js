...
if(response === 'left'){
    ...
    if(response2 === 'follow'){
        ...
        if (response3==='stay'){ let response4 =window.alert("You live happily amongst the cats for the rest of your days.");}
        else if (response3==="spread the word"){
            let response4= window.alert("After leaving the cat colony, you are never able to find it again; without proof, no one believes your story, which passes into legend nonetheless.");
        }
        ...
        let response3= window.prompt("You come across a chamber that reaches upward to a shining light above. There is a long, winding staircase, and a much quicker, but rickety-looking ladder that leads up toward the light. Which do you take? Type ladder or staircase.");
         if (response3==="ladder"){ let response3 = window.alert('After ascending a few feet up the ladder, one of its rungs snaps, and you comedically fall through each of the rungs below. Sheepish, you return home.');
    }
        else if (response3==="staircase"){let response3= window.alert ('After ascending the staircase, you discover a shiny blue stone, which you take home and cherish forever.');
...
if (response3==="stay"){ let response4= window.alert ('You and the dragon have an uplifting conversation about local politics and become lifelong friends.');
}

 else if (response3 === "run") {
 let response3= window.alert ('Quickly, you run back to the caves entrance. Sheepish, you return home.');
 ...
 if (response3==="draw it"){
     let response3= window.alert("You draw the flower, capturing only a fraction of its beauty with your quill. You bring the drawing home, somewhat disappointed, but over time, discover joy in sharing it with your friends and family, recounting the story of your days as a sorcerer with the aid of the sketch.");

}
else if (response3==="pick it"){let responsee3= window.alert("You pick the flower and bring it home, and all marvel at its brilliance. However, over time it fades and eventually crumbles to dust.")}
}
}
